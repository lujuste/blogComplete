...
const timeRead = Math.ceil(totalWords / 200)
  ...
  const isPostEdit = post.first_publication_date !== post.last_publication_date 
  let editOnDate

  if (isPostEdit) {
    editOnDate = format(
      new Date(post.last_publication_date), "'* editado em' dd MMM yyyy', às' h':'m", 
      {
        locale: ptBR,
      }
    )
